import { btnPrevNextDisable } from './module/_btn-prev-next-disable.js'
import { renderUserList } from './module/_render-user-list.js'
import { CreateNewUser } from './module/_create-new-user.js'
import { validLogin } from './module/_valid-login.js'
import { login } from './module/_login-component.js'
import {
...
} from './module/_form-input-create.js'
import { renderUpdateUser, renderUpdateNewUser } from './module/_render-update-user.js'
...
formLogin.addEventListener('submit', (e) => {
    e.preventDefault();
     if (
         ...
         login(inputEmailLogin, inputPass, succsesAuthorization, messageFinal);
     }
});
...
function requestUpdUser(li) {
    ...
    inputNewFirstName.value = li.children[0].innerHTML.split(' ')[0];
    inputNewLastName.value = li.children[0].innerHTML.split(' ')[1];
    inputNewEmail.value = li.children[1].innerHTML;
    inputNewJob.value = li.children[2].innerHTML;
 ...
 function upDateN(name1, name2, email, job){
    ...
    .then((response) => {
        const responsNew = response;
         formUpdate.remove();
         btnCancelUpdate.remove(); 
         renderUpdateNewUser(responsNew, li); 
    })
    .catch(error => console.log(error))
};
};

const requestUpdUserServer = function(li) {
    ...
    inputNewFirstName.value = li.children[1].innerHTML.split(' ')[0];
    inputNewLastName.value = li.children[1].innerHTML.split(' ')[1];
    inputNewEmail.value = li.children[2].innerHTML;
